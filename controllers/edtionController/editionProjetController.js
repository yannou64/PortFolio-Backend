...
export async function updateProjet(req, res) {
  ...
  try {
    await Projet.findByIdAndUpdate(id, updatedProjet, { runValidators: true });
...
function sanitization(data) {
  return typeof data === "string" // On ne peut faire des replaces que sur des strings
    ? data
